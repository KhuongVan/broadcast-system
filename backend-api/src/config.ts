...
export const config = {
  ...
  recordingProofEnabled: (process.env.RECORDING_PROOF_ENABLED || 'true').toLowerCase() !== 'false',
  recordingProofSegmentSeconds: Number(process.env.RECORDING_PROOF_SEGMENT_SECONDS || 900),
  recordingProofPaddingBeforeSeconds: Number(process.env.RECORDING_PROOF_PADDING_BEFORE_SECONDS || 60),
  recordingProofPaddingAfterSeconds: Number(process.env.RECORDING_PROOF_PADDING_AFTER_SECONDS || 60),
  recordingProofRetentionDays: Number(process.env.RECORDING_PROOF_RETENTION_DAYS || 90),
  recordingProofAudioProfile: process.env.RECORDING_PROOF_AUDIO_PROFILE || 'VOICE_STANDARD',
